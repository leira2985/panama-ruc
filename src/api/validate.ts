 ...
 */
export function validate(input: string, expectedDv?: string): ValidateResult {
  ...
  // Parsear y calcular
  const result = safeParse(rucPart);
  if (!result.ok) {
 ...
 */
export function isValid(input: string, expectedDv?: string): boolean {
  return validate(input, expectedDv).valid;
}
